...
Click element when ready
    ...
    [Arguments]     ${locator}  ${retry}=4      ${only_contains}=${FALSE}       ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ...
    FOR     ${i}    IN RANGE    1   ${retry}
        ...
        ELSE
            ${enabled_status}=    Run keyword and ignore error    SeleniumLibrary.Wait until element is enabled    ${locator}    ${timeout}
            ${visible_status}=    Run keyword and ignore error    SeleniumLibrary.Wait until element is visible    ${locator}    ${timeout}
            ${wait_status}=       Set variable    ${enabled_status}
            Run keyword if        '${enabled_status[0]}'!='PASS' and '${visible_status[0]}'=='PASS'    Set variable    ${wait_status}    ${visible_status}
            ${err_msg_wait}=            Convert to string       ${wait_status[1]}
            ${is_not_stale_wait}=       Run keyword and return status    Should not contain     ${err_msg_wait}      StaleElementReferenceException
        END
    ...
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Failed to click element after ${retry} retry with click error "${err_msg}", and wait error "${err_msg_wait}"
    Should be true      ${result}       msg=${msg}
...
Input text to element when ready
    ...
    [Arguments]     ${locator}     ${text}     ${clear}=${TRUE}     ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ${visible_status}=    Run keyword and ignore error    SeleniumLibrary.Wait until element is visible    ${locator}    ${timeout}
    ${enabled_status}=    Run keyword and ignore error    SeleniumLibrary.Wait until element is enabled    ${locator}    ${timeout}
    ${is_success}=    Evaluate    '${enabled_status[0]}'=='PASS' and '${visible_status[0]}'=='PASS'
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Element ${locator} is not found
    Should be true      ${is_success}       msg=${msg}
    ...
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Unable to input text to element after 4 retry with error "${err_msg}"
    Should be true      ${is_success}       msg=${msg}

Select option by label when ready
    [Documentation]     Wait until element is visible first before select from list by label
    [Arguments]     ${locator}     ${label}     ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ${visible_status}=    Run keyword and ignore error    DobbyWebCommon.Wait until element is visible except stale    ${locator}    failure_msg=${failure_msg}
    ${is_success}=    Evaluate    '${visible_status[0]}'=='PASS'
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Element ${locator} is not found
    Should be true      ${is_success}       msg=${msg}
    FOR    ${index}    IN RANGE    1    10
        ...
        Exit For Loop If        ${is_success} or ${is_not_loading_error}
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Unable to select option dropdownlist after 10 retry with error "${err_msg}"
    Should be true      ${is_success}       msg=${msg}
...
Get text from element when ready
    ...
    [Arguments]     ${locator}      ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ${visible_status}=    Run keyword and ignore error    DobbyWebCommon.Wait until element is visible except stale    ${locator}    failure_msg=${failure_msg}
    ${is_success}=    Evaluate    '${visible_status[0]}'=='PASS'
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Element ${locator} is not found
    Should be true      ${is_success}       msg=${msg}
...
Get element attribute when ready
    ...
    ...     \n default timeout is same as ${GLOBAL_TIMEOUT}
    [Arguments]     ${locator}      ${attribute}    ${only_contain}=${FALSE}    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    IF  ${only_contain}
        Wait until keyword succeeds    5x    2s     DobbyWebCommon.Wait until page contains element except stale    ${locator}      ${timeout}    failure_msg=${failure_msg}
    ELSE IF     ${only_contain} == ${FALSE}
        Wait until keyword succeeds    5x    2s     DobbyWebCommon.Wait until element is visible except stale       ${locator}      ${timeout}    failure_msg=${failure_msg}
    END
...
Scroll element into view when ready
    ...
    ...     \n default timeout is same as ${GLOBAL_TIMEOUT}
    [Arguments]     ${locator}      ${retry}=4      ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    FOR     ${i}    IN RANGE   0    ${retry}
        DobbyWebCommon.Wait until page contains element except stale    ${locator}      ${timeout}    failure_msg=${failure_msg}
        SeleniumLibrary.Scroll element into view            ${locator}
        ${is_visible}=      Run keyword and return status       DobbyWebCommon.Wait until element is visible except stale       ${locator}      ${timeout}    failure_msg=${failure_msg}
...
Browse file when ready
    ...
    ...     \n default timeout is same as ${GLOBAL_TIMEOUT}
    [Arguments]     ${locator}      ${file_path}    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    DobbyWebCommon.Wait until page contains element except stale   ${locator}       ${timeout}    failure_msg=${failure_msg}
...
Get value from element when ready
    ...
    ...     \n default timeout is same as ${GLOBAL_TIMEOUT}
    [Arguments]     ${locator}  ${only_contain}=${FALSE}    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    IF  ${only_contain}
        DobbyWebCommon.Wait until page contains element except stale    ${locator}      ${timeout}    failure_msg=${failure_msg}
    ELSE IF     ${only_contain} == ${FALSE}
        DobbyWebCommon.Wait until element is visible except stale       ${locator}      ${timeout}    failure_msg=${failure_msg}
    END
...
Manually clear input from textbox
    ...
    [Arguments]     ${locator}      ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ${current_value}=   DobbyWebCommon.Get value from element when ready    ${locator}      ${timeout}    failure_msg=${failure_msg}
...
Wait until element is visible except stale
    ...
    [Arguments]    ${locator}       ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    FOR     ${i}    IN RANGE    1       15
        ${stale_status}           Run keyword and ignore error   SeleniumLibrary.Wait until element is visible    ${locator}      ${timeout}
        ${stale_err_msg}=         Convert to string       ${stale_status[1]}
        ${is_not_stale}=    Run keyword and return status    Should not contain     ${stale_err_msg}      StaleElementReferenceException
        Exit For Loop If        ${is_not_stale}
    END
    ${visible_status}=     Run keyword and ignore error    SeleniumLibrary.Wait until element is visible    ${locator}    ${timeout}
    ${visible_err_msg}=       Convert to string     ${visible_status[1]}
    ${no_err_msg}=       Run keyword and return status    Should be equal       ${visible_status[0]}      PASS    msg=${visible_err_msg}
    ${both_status}=     Evaluate    ${is_not_stale} and ${no_err_msg}
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Element is not visible and still in stale stage with error "${visible_err_msg}"
    Should be true      ${both_status}       msg=${msg}

Wait until page contains element except stale
    [Documentation]  Wait until page contains element except stale
    ...     \n Retries if a StaleElementReferenceException occurs.
    ...     \n Default timeout is same as ${GLOBAL_TIMEOUT}.
    [Arguments]    ${locator}    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    FOR    ${i}    IN RANGE    1    15
        ${stale_status}=    Run keyword and ignore error    SeleniumLibrary.Wait until page contains element    ${locator}    ${timeout}
        ${stale_err_msg}=   Convert to string    ${stale_status[1]}
        ${is_not_stale}=    Run keyword and return status    Should not contain    ${stale_err_msg}    StaleElementReferenceException
        Exit For Loop If    ${is_not_stale}
    END
    ${contain_status}=    Run keyword and ignore error    SeleniumLibrary.Wait until page contains element    ${locator}    ${timeout}
    ${contain_err_msg}=   Convert to string    ${contain_status[1]}
    ${no_err_msg}=        Run keyword and return status    Should be equal    ${contain_status[0]}    PASS    msg=${contain_err_msg}
    ${both_status}=       Evaluate    ${is_not_stale} and ${no_err_msg}
    ${msg}=    GeneralUtils.Format failure message    ${failure_msg}    Full error message is: Page does not contain element (still stale or missing) with error "${contain_err_msg}"
    Should Be True    ${both_status}    msg=${msg}
...
Click element at coordinates when ready
    ...
    ...     \n default timeout is same as ${GLOBAL_TIMEOUT}
    [Arguments]     ${locator}      ${xoffset}  ${yoffset}      ${retry}=4      ${only_contains}=${FALSE}       ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ...
    END
    ${msg}=    GeneralUtils.Format failure message    ${failure_msg}    Full error message is: Failed to click element after ${retry} retry with error message "${err_msg}"
    Should be true    ${result}    msg=${msg}
...
Click image on screen
    ...
    [Arguments]     ${abs_expected_image_path}      ${threshold}=0.8    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    ${is_found_image}   ${xy}=  DobbyWebCommon.Image should be visible on screen   ${expected_image}   ${threshold}=0.8

    IF      ${is_found_image}
        DobbyWebCommon.Click element at coordinates when ready      body    ${xy[0]}     ${xy[1]}   ${timeout}    failure_msg=${failure_msg}
    ELSE
        ${msg}=    GeneralUtils.Format failure message    ${failure_msg}    Full error message is: Image was not found on the screen
        Should be true    ${is_found_image}    msg=${msg}
    END
...
Mouse over when ready
    ...
    [Arguments]     ${locator}    ${failure_msg}=${NONE}
    DobbyWebCommon.Wait until element is visible except stale       ${locator}    failure_msg=${failure_msg}
...
Verify element text when ready
    ...
    [Arguments]     ${locator}      ${expected}     ${override_error_msg}=${None}    ${ignore_case}=${False}       ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    DobbyWebCommon.Wait until element is visible except stale       ${locator}      ${timeout}    failure_msg=${failure_msg}
...
Verify element should contains text when ready
    ...
    [Arguments]     ${locator}      ${expected}     ${override_error_msg}=${None}    ${ignore_case}=${False}       ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    DobbyWebCommon.Wait until element is visible except stale       ${locator}      ${timeout}
...
Scroll to element
    ...
    [Arguments]    ${locator}    ${block}=center    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    DobbyWebCommon.Wait until page contains element except stale    ${locator}    ${timeout}    failure_msg=${failure_msg}
