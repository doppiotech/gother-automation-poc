...
Init mobile farm configuration
    [Documentation]  Init doppio mobile farm, Use hand in hand with woody mobile farm management
    ...
    ${doppioAutomationName}=    Get value from set     automationName
    ${doppioPlatforName}=       Get value from set     platformName
    ${doppioPlatformVersion}=   Get value from set     platformVersion
    ${doppioDeviceName}=        Get value from set     deviceName
    ${doppioUdid}=              Get value from set     udid
    ${doppioRemoteUrl}=         Get value from set     remote_url
    ${doppioSystemPort}=        Get value from set     system_port
    ${doppioSystemPort_int}=    Convert to integer      ${doppioSystemPort}
    Set global variable     ${automationName}           ${doppioAutomationName}
    Set global variable     ${platformName}             ${doppioPlatforName}
    Set global variable     ${platformVersion}          ${doppioPlatformVersion}
    Set global variable     ${deviceName}               ${doppioDeviceName}
    Set global variable     ${udid}                     ${doppioUdid}
    Set global variable     ${remote_url}               ${doppioRemoteUrl}
    Set global variable     ${system_port}              ${doppioSystemPort_int}
    Release Value Set

Set resolution
    [Documentation]     Set device resolution according to scroll element size
    [Arguments]       ${scroll_element}     ${percent}=0.3    ${failure_msg}=${NONE}
    DobbyAppCommon.Wait until element is visible except stale    ${scroll_element}    failure_msg=${failure_msg}
...
Clear text when ready
    ...
    DobbyAppCommon.Wait until element is visible except stale       ${locator}    ${timeout}    failure_msg=${failure_msg}
...
Tap element when ready
    ...
    ...    \n failure_msg - failure message that will appear in error log when test(s) is failed

    [Arguments]     ${locator}      ${only_contain}=${FALSE}    ${timeout}=${GLOBAL_TIMEOUT}    ${wait_until_ready}=${TRUE}    ${max_attempts}=4    ${failure_msg}=${NONE}
    FOR     ${i}    IN RANGE    1   ${max_attempts}
        ...
        ${tap_status}=              Run keyword and ignore error   AppiumLibrary.Click element         ${locator}
    ...
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Unable to tap after ${${max_attempts}-1} attempts, with tap error "${err_msg_tap}", and wait error "${err_msg_wait}",
...
Long press when ready
    ...
    [Arguments]     ${locator}      ${duration}=1000    ${timeout}=${GLOBAL_TIMEOUT}    ${failure_msg}=${NONE}
    DobbyAppCommon.Wait until element is visible except stale     ${locator}    ${timeout}    failure_msg=${failure_msg}
...
Input text to element when ready
    ...
    ...             ${wait_until_ready}=${TRUE}        ${max_attempts}=4    ${failure_msg}=${NONE}
    DobbyAppCommon.Tap element when ready   ${locator}     timeout=${timeout}    wait_until_ready=${wait_until_ready}    max_attempts=${max_attempts}    failure_msg=${failure_msg}
    IF  ${clear}
        AppiumLibrary.Clear text      ${locator}
    END
...
Hide keyboard until success
    ...
    [Arguments]    ${failure_msg}=${NONE}
    FOR     ${i}    IN RANGE    1   5
        AppiumLibrary.Hide keyboard
        ${is_shown}=    AppiumLibrary.Is Keyboard Shown
        Exit for loop if    '${is_shown}' == 'False'
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Keyboard is still shown after 10 retry
    Should be true      ${is_shown}       msg=${msg}
...
Find element
    ...
    [Arguments]         ${target_element}
                ...
                ...     ${failure_msg}=${NONE}
    ${element_status}=      Run keyword and return status    DobbyAppCommon.Wait until element is visible except stale      ${target_element}      ${timeout}    failure_msg=${failure_msg}
    IF  '${element_status}'=='False'
        ...
        END
        ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Element is not visible after ${number_of_scroll} scroll
        Should be true      ${element_status}       msg=${msg}
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: Cannot find element ${target_element}
    Should be true      ${element_status}       msg=${msg}

Wait until element is visible except stale
    [Documentation]         Wait until element is visible except stale
    [Arguments]    ${locator}        ${timeout}=${GLOBAL_TIMEOUT}    ${max_attempts}=5    ${failure_msg}=${NONE}
    FOR     ${i}    IN RANGE    1       ${max_attempts}
        ...
        Exit for loop if        ${result}
    END
    ${msg}=    GeneralUtils.Format failure message     ${failure_msg}     Full error message is: ${err_msg}
    Should be true      ${result}       msg=${msg}
...
Click image on screen
    ...
    [Arguments]     ${abs_expected_image_path}      ${threshold}=0.8    ${failure_msg}=${NONE}
    ${is_found_image}   ${xy}=  DobbyAppCommon.Image should be visible on screen   ${expected_image}   ${threshold}=0.8
