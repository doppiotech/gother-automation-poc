...
Get secrets from Azkaban
    [Documentation]     Return secret list in json form
    ...     \n support getting secrets from project_uuid, folder_uuid, secret_uuid, secret_key
    ...     \n AZAKABAN_URL, AZAKABAN_USERNAME, AZAKABAN_PASSWORD env are required for authentication
    [Arguments]     ${project_uuid}=${NONE}        ${folder_uuid}=${NONE}   ${secret_uuid}=${NONE}      ${secret_key}=${NONE}
    ${secrets}=     AzkabanSecrets.Get secrets from Azkaban     ${project_uuid}     ${folder_uuid}      ${secret_uuid}      ${secret_key}
    RETURN    ${secrets}