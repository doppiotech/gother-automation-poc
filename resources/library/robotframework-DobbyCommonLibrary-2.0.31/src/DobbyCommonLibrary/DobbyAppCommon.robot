...
Set resolution
    ...
    DobbyAppCommon.Wait until element is visible except stale    ${scroll_element}
...
Clear text when ready
    ...
    DobbyAppCommon.Wait until element is visible except stale       ${locator}    ${timeout}
...
Input text to element when ready
    ...
    DobbyAppCommon.Tap element when ready   ${locator}     timeout=${timeout}    wait_until_ready=${wait_until_ready}    max_attempts=${max_attempts}
    IF  ${clear}
        AppiumLibrary.Clear text      ${locator}
    END
...
Find element
    ...
    ${element_status}=      Run keyword and return status    DobbyAppCommon.Wait until element is visible except stale      ${target_element}      ${timeout}
